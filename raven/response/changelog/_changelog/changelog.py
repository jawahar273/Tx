...
class Changelog(BaseResponse):
    ...
    def render(self, pretty=None):
        ...
        super(Changelog, self).render(class_name=self.class_name, sub_path="changelog")
        ...
        if changelog_list.status_code != 200:
            changelog_list = [f"Error in getting result: {changelog_list.status_code}"]
        elif changelog_list.status_code == 200:
            if len(changelog_list) == 0:
                changelog_list = ["No result to display"]

        return self.render_template.render(
            pretty=pretty, domain="https://changelogs.md", changelog_list=changelog_list
        )
